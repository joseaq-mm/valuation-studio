...
- **✅ COMUNIDAD · FASE 4 — IA de conclusiones (7 ago 2026, testing_agent iteration_39 100% · backend 7/7 pytest + frontend e2e · modelo Claude Sonnet 4.6):**
  ...
  - **Generación bajo demanda + caché**: los resultados se cachean (`community_ai` id="signal", `feedback_ai` id="insights"); `GET .../ai` y `GET .../insights` devuelven la última versión sin gastar IA. Botón "Actualizar" regenera. Fail-open → 502 con aviso si la IA no responde.
  - **Backend**: helper compartido `services/llm.py` (`claude_json`, anthropic/`claude-sonnet-4-6`, Emergent LLM Key). Endpoints: `GET/POST /api/community/ideas/signal/ai|generate`, `GET/POST /api/feedback/admin/insights[/generate]`. Regression: `/app/backend/tests/test_fase4_ai_insights.py` (7/7).
  - Nota: el generate de la señal NO está gated a admin (feature de comunidad); posible rate-limit a futuro si hay abuso.

- **✅ COMUNIDAD · FASE 3 — Buscador global + Moderación + Anti-manipulación (7 ago 2026, testing_agent iteration_38 100% · backend 10/10 pytest + frontend e2e):**
  - **Buscador global** en `/comunidad` (`community-search-input`): Mongo **text index** sobre `community_topics` (title+body) y `community_replies` (body) creado al arranque en `server.py`. Debounce 300ms, mín 2 chars, desplegable `community-search-results` agrupado (temas/ideas/respuestas), `community-search-clear` limpia, `search-no-results` si sin coincidencias. Excluye `is_removed`/`hidden`.
  - **Botón Reportar** (`components/community/ReportButton.jsx`, `report-btn-<id>`) en cada tema/respuesta → popover con motivo (`report-reason`) + `report-send`. `POST /api/community/report` {target_type, target_id, reason}. Dedupe por usuario (mismo usuario → `already:true`). **Auto-oculta (hidden=true) al recibir 3 reportes de usuarios DISTINTOS** (community.py L610).
  - **Cola de moderación Admin** (pestaña "Moderación" en `/admin/feedback`, `admin-moderation`): `GET /api/community/admin/moderation?status=open|all` (solo admin, 403 no-admin). Cada item con contador de reportes / badge IA, botón **Aprobar** (`approve` → restaura hidden=false) y **Borrar** (`delete` → is_removed=true). `POST /api/community/admin/moderation/{mid}` {action}.
  - **IA anti-manipulación** al publicar una idea (`assess_idea`, Emergent LLM Key): contrasta el claim (target_price/postura) contra la valoración propia (POV/ratios); si es sospechoso, inserta un flag `kind=ai` en la cola (fail-open). Verificado: idea con objetivo 2000 vs POV 437 → flag "Objetivo muy por encima de tu POV".
  - **Backend regression:** `/app/backend/tests/test_community_moderation.py` (10/10).
  - Mejoras no bloqueantes anotadas (review): `assess_idea` corre síncrono tras insert (podría ir a background); umbral 3 hard-coded; `community.py` ~684 líneas (P3 split).
...
- **✅ COMUNIDAD · FASE 0 — cimientos + consolidación de cabecera (6 ago 2026, verificado curl + screenshot):**
  ...
  - **Se conserva TODO** lo del sistema de Sugerencias/Admin/Encuestas.

- **✅ SISTEMA DE SUGERENCIAS/FEEDBACK BIDIRECCIONAL + ENCUESTAS (6 ago 2026, testing_agent iteration_34 100% · backend 25/25 pytest + frontend e2e):**
  - **Solo usuarios logueados.** Cabecera (a petición del usuario): botón **"Sugerencias"** (`feedback-open-btn`) + **campana** (`feedback-bell-btn`) con badge de no-leídos (`feedback-unread-badge`/`feedback-bell-badge`); si el usuario es admin, además icono escudo → enlace a `/admin/feedback` (`feedback-admin-link` + `feedback-admin-badge`). Componente `components/feedback/FeedbackControls.jsx` (sondea `/api/feedback/unread` cada 45s). Montado en `Layout.jsx` (fila utilidades desktop + fila compacta móvil).
  - **Panel de usuario** (`components/feedback/FeedbackPanel.jsx`, slide-over derecho): 3 pestañas — **Enviar** (tipo Error/Idea/Mejora `feedback-type-*`, mensaje, **captura opcional con html2canvas** `feedback-capture-btn` que ignora el propio panel vía `data-fb-ignore`, y **metadatos automáticos** navegador/SO/viewport/idioma/página vía `lib/feedbackMeta.js`), **Mis mensajes** (hilos tipo chat, responder), **Encuestas** (votar encuestas globales activas con % de resultados).
  - **Panel Admin** (`pages/AdminFeedback.jsx`, ruta `/admin/feedback`, guard por `is_admin` desde `/feedback/unread`; no-admin → `admin-feedback-denied`): **buscador** por asunto/email/nombre (`admin-search-input`), filtros tipo/estado, lista de hilos con badge de no-leídos, vista de hilo con **metadatos** (`admin-thread-meta`), captura, respuesta y cambio de **estado** (nuevo/en_curso/resuelto). Sección **Encuestas globales**: crear (pregunta + N opciones), publicar, ver resultados y **activar/cerrar** (`admin-survey-toggle-*`).
  - **Backend** `routes/feedback.py` (registrado en `server.py`; `make_router(db, auth_required)`): `POST/GET /api/feedback/threads`, `GET/POST /api/feedback/threads/{id}[/messages|/screenshot]`, `GET /api/feedback/unread`, `GET /api/feedback/admin/threads?q=&type=&status=`, `PATCH /api/feedback/admin/threads/{id}`, `POST/GET /api/feedback/surveys`, `POST /api/feedback/surveys/{id}/vote`, `PATCH /api/feedback/surveys/{id}`. Admin = email == `ADMIN_EMAIL` (backend/.env = joseaq.2m@gmail.com). Capturas en object storage. Colecciones: `feedback_threads`, `feedback_messages`, `surveys`, `survey_votes` (+índices en server.py). Un hilo resuelto se reabre (in_progress) si el usuario responde.
  - Aviso externo al admin (Webhook/Resend) al llegar feedback → DIFERIDO por el usuario (P2).

- **✅ COMPARTIR + AJUSTES (3 ago 2026, testing_agent iteration_33 100% funcional):**
  - **Ajustes** (`components/ThresholdsDialog.jsx`): el botón de cabecera antes "Umbrales" ahora es **"Ajustes"** (data-testid `settings-open`/`settings-modal`) con 3 secciones: **Moneda** (`CurrencySelector`, retirado de la cabecera), **Compartir/Integraciones** (webhooks entrantes de **Discord** y **Slack**: pegar URL + Guardar + Probar + Eliminar, `WebhookRow`), y **Umbrales** (lo de antes). La URL del webhook se guarda por usuario y se muestra enmascarada.
  - **Botón Compartir** (`components/ShareMenu.jsx`) a la IZQUIERDA de cada control de descarga: tesis, snapshot KPI, documentos KPI, gráficos (Visual, Macro trend+coef-history, Company chart+POC/POV), treemap, y PDF de página en cabecera. En tesis/docs KPI comparte el **formato seleccionado** (vía `leftSlot(fmt)` de `FormatDownloadMenu`).
  - Opciones del panel: (1) copiar enlace + **QR** (`qrcode.react`), (2) compartir nativo (`navigator.share`), (3) **WhatsApp/Telegram/X/Facebook/Email** (intents), (4) **Discord/Slack** (POST a webhook del usuario). El panel detecta el borde del viewport (`alignLeft`) para no salirse.
  - **Enlaces públicos con caducidad de 15 días**: `routes/share.py` genera el doc (o recibe blob del cliente), lo guarda en object storage y crea `shared_docs` con token. Visor público **sin login** en `/s/:token` (`pages/ShareView.jsx`, ruta fuera del Layout en App.js): previsualiza PDF (iframe) y JPG (img) y ofrece descarga; Word (.docx) es solo-descarga. Compartir requiere login (a anónimos: toast "Inicia sesión para compartir").
  - Backend endpoints: `GET/PUT /api/settings`, `POST /api/settings/test-webhook`, `POST /api/share/create` (kinds: thesis|kpi_snapshot|kpi_file), `POST /api/share/upload` (blobs: gráficos JPG / PDF de página), `POST /api/share/webhook`, `GET /api/share/{token}`, `GET /api/share/{token}/file` (`?dl=1` fuerza descarga). Webhooks validados por host allowlist (discord.com / hooks.slack.com), solo https. Dep nueva frontend: `qrcode.react`.
  - Colecciones nuevas: `user_settings` (webhooks por usuario), `shared_docs` (token, formato, storage_path, expires_at 15d).

- **✅ DESCARGA MULTIFORMATO DE DOCUMENTOS KPI SUBIDOS (3 ago 2026, verificado curl + Playwright):**
  - En cada documento de `components/kpi/KpiDocuments.jsx`, el antiguo icono de descarga es ahora un **botón partido** (`FileDownloadMenu`): icono del formato actual + flecha `ChevronDown`. Clic en el icono descarga en el formato seleccionado; la flecha abre un menú (PDF · Word · JPG) que al elegir uno cambia el icono y descarga.
  - **Convierte el documento ORIGINAL** (no la lectura de la IA) al formato elegido. JPG solo se ofrece si el documento ya es una imagen (`content_type` image/*); en PDF/transcript solo PDF y Word. "Doc de Google" = el mismo `.docx` (se abre en Google Docs sin integración extra).
  - Formatos por tipo: imagen → [jpg(def), pdf, docx]; pdf → [pdf(def), docx]; texto/transcript → [pdf(def), docx].
  - Backend: `GET /api/thesis/{company_id}/kpis/files/{file_id}/export/{fmt}` (fmt ∈ pdf|docx|jpg). Conversores en `services/doc_export.py`: `image_to_jpg`/`image_to_pdf`/`image_to_docx` (Pillow+reportlab+python-docx), `pdf_to_docx` (PyMuPDF/fitz renderiza cada página a imagen, máx 40, y las incrusta), `text_to_pdf`/`text_to_docx`. Nueva dependencia: **pymupdf==1.28.0** (en requirements.txt).
  - Verificado: imagen→jpg/pdf/docx, pdf→pdf/docx (jpg rechazado 400 correctamente), texto→pdf/docx, todos HTTP 200 con bytes válidos; UI: descarga por icono y por menú, el icono cambia al formato elegido. data-testids: `kpi-file-download-{id}`, `kpi-file-download-caret-{id}`, `kpi-file-format-{fmt}-{id}`.
- **✅ FIX JPG en gráficos con leyenda (3 ago 2026):** `lib/chartExport.js` ahora usa `pickChartSvg()` (elige el `<svg>` de mayor área) en vez del primer svg. Antes, en gráficos con `<Legend>` de recharts (Macro «Evolución 10 años» y POC/POV) cogía el iconito de leyenda 14×14 → JPG diminuto. Verificado: evolución 1960×920, POC/POV 1948×1040.

- **✅ DESCARGAS/EXPORTACIÓN — 4 tareas (3 ago 2026, testing_agent iteration_32 100% · 12/12 flujos):**
  1. **Tesis → PDF/DOCX**: componente reutilizable `components/thesis/ThesisDownloadButtons.jsx` (botones "Descargar PDF"/"Descargar Word" SIEMPRE visibles bajo el título) en `ThesisResult.jsx` (gated `thesis.id`) y `TendenciaResult.jsx` (gated `tendencia.id`, aparece al guardar). API `thesisExport(id,fmt)` → `GET /api/thesis/{id}/export/{fmt}` (backend `services/doc_export.py` con reportlab + python-docx). data-testids: `thesis-download-pdf`, `thesis-download-docx`.
  2. **KPIs → PDF/DOCX**: el snapshot de validación (coeficiente + drivers + tabla + fuentes) se descarga desde la cabecera del coeficiente global en `Kpis.jsx` (botones "PDF"/"WORD"). API `kpiExport(companyId,fmt)` → `GET /api/thesis/{company_id}/kpis/export/{fmt}` (nueva `kpi_snapshot_blocks`/`kpi_snapshot_pdf`/`kpi_snapshot_docx` en `doc_export.py`). Los DOCUMENTOS SUBIDOS (PDF/imagen) ya se descargaban como original vía `kpiFileDownload` (`KpiDocuments.jsx`, sin cambios). data-testids: `kpi-download-pdf`, `kpi-download-docx`.
  3. **Gráficos ampliados → JPG (retina 2x)**: util `lib/chartExport.js` — `downloadSvgJpg(container, filename)` serializa el `<svg>` de Recharts a canvas 2x (fondo blanco, fuente IBM Plex) y `downloadHtmlJpg(element, filename)` usa `html2canvas` (añadido a package.json) para el treemap (divs HTML). Botón "JPG" en TODOS los modales a pantalla completa: Visual (`visual-chart-download-jpg`), Macro TrendModal (`trend-modal-download-jpg`) y CoefHistory (`coef-history-download-jpg`), Company ChartBlock (`<testid>-modal-download-jpg`) y RatioHistory (`ratio-history-modal-download-jpg`), treemap `ThesisExplore` (`explore-treemap-download-jpg`).
  4. **Página actual → PDF (1 clic)**: `components/PageExportButton.jsx` (data-testid `page-export-pdf`) en la barra de utilidades de la cabecera (`Layout.jsx`, escritorio + drawer móvil). Usa `html2pdf.js` sobre `<main>`; nombre derivado de la ruta (`valuation-studio-{slug}.pdf`).
  - Verificado (testing_agent): 12/12 descargas disparan evento con extensión y nombre correctos; 6/6 páginas sin errores. Backend verificado por curl (PDF `%PDF-`, DOCX zip válido con 3 tablas).

- **✅ Company modales landscape v2 (3 ago 2026): sin scroll + centrado + leyenda pegada.** Sobre el fix anterior: (1) en landscape el modal pasa a altura definida `h-[95vh]` + `overflow-hidden` y el contenedor del gráfico usa `flex-1 min-h-0` (en vez de `h-[70vh]` fijo) → ocupa justo el espacio restante, gráfico centrado y SIN scroll vertical residual (verificado scrollHeight==clientHeight). (2) En RatioHistoryChart (POC/POV) se acercó la leyenda al eje X: `XAxis height={big?22:18}` + `Legend height={big?22:18} wrapperStyle paddingTop:0` → gap ~16px (antes ~30px). `Company.jsx`.
- **✅ FIX bug rotación "media pantalla izquierda" (3 ago 2026):** tras abrir+cerrar un gráfico a pantalla completa (`fixed inset-0`) en horizontal y girar a vertical, algunos navegadores Android/Chromium mantenían el ancho del *layout viewport* horizontal → toda la página se veía reducida (~0.46×) pegada a la izquierda. No era CSS (emulación por resize NO lo reproduce; DOM vuelve a 390px sin estilos residuales) sino el reseteo de escala del navegador móvil. FIX (`components/Layout.jsx`, global): en `orientationchange` se añade `maximum-scale=1` a la meta viewport un instante (fuerza recomputar ancho/escala) y se restaura a los 350ms (mantiene pinch-zoom). Verificado el toggle+restauración de la meta; el efecto real solo se comprueba en dispositivo físico.
- **✅ Cartera (Nivel 1) — 3 cambios (3 ago 2026, verificado screenshot):** (1) eliminadas las 4 tarjetas KPI (Invertido/Valor/P&L/P&L%) de la vista tabla (`Portfolio.jsx`). (2) `PortfolioDonut.jsx`: totales **encima del anillo** (Invertido (compra) · Valor total actual · P/L · P/L%) y cada fila de la leyenda muestra SOLO el porcentaje (quitado el importe). (3) Botón **"Ocultar cartera"** con icono ojo/ojo-tachado junto al tag "Nivel I" (`hideMoney` en localStorage `vs:portfolio-hide-money`): desenfoca (`blur-sm select-none`) los importes de dinero de la tabla (acciones, compra, invertido, valor, P/L) y del donut (totales monetarios + centro del anillo); porcentajes y datos públicos (precio, señales, P/L%) quedan visibles.
- **✅ Chat KPI: recuperar/editar/guardar/refrescar conversación (3 ago 2026, backend curl-verified + UI screenshot):** 4 botones en `KpiChat.jsx` (orden: **Mostrar último chat · Guardar chat · Refrescar chat · Guardar como documento**, con tooltips `HoverTip`).
  - **Mostrar último chat**: `GET /thesis/{cid}/kpis/chat/last` → carga la conversación más reciente (o aviso si no existe) y conmuta la sesión activa.
  - **Editar/borrar**: cada bloque tiene borrar; las preguntas del usuario tienen editar en línea. Cambios locales hasta "Guardar chat".
  - **Guardar chat**: `PUT /thesis/{cid}/kpis/chat` {session_id, messages} → sobrescribe los mensajes de la conversación.
  - **Refrescar chat**: `ConfirmDialog` (avisa tiempo/saldo IA) + barra de progreso; relanza las preguntas del usuario a una sesión NUEVA vía el `POST /kpis/chat` existente (contexto/datos actuales), regenera respuestas.
  - **Guardar como documento**: persiste el estado actual (PUT) y luego `kpiChatSave` (fuente KPI) — igual que antes pero recogiendo lo editado.
  - Backend: `routes/thesis.py` (modelo `KpiChatReplaceRequest`, endpoints `chat/last` GET y `chat` PUT). API: `kpiChatLast`, `kpiChatReplace`.
  - NOTA test: para probar la UI se marcó `thesis_test_company_msft` como completa (se le añadió `split_dev`); re-ejecuta `seed_company_thesis_test.py` para resetear.
  - NO se lanzó un "Refrescar" real (evita consumir saldo IA); reutiliza el endpoint de chat ya probado.
- **✅ Chat KPI v2 (3 ago 2026): botón "Borrar chat" + título de documento con sentido.** (1) Nuevo botón **Borrar chat** entre "Mostrar último chat" y "Guardar chat" (`KpiChat.jsx`, `clearChat`): vacía el chat en pantalla y abre sesión nueva para empezar de cero; NO borra la conversación guardada (recuperable con "Mostrar último chat"). Orden barra: Mostrar último chat · Borrar chat · Guardar chat · Refrescar chat · Guardar como documento. (2) FIX título al "Guardar como documento" (`routes/thesis.py` `kpi_chat_save`): antes truncaba la 1ª pregunta a 60 chars cortando a media palabra; ahora corta por límite de palabra (~55) + "…" e incluye el ticker → "Chat KPIs · {TICKER} · {pregunta…}". Verificado por curl + screenshot.
## Implementado (Feb 2026)
- **✅ Company: modales de gráficos responsive en horizontal (3 ago 2026, verificado 900x400):** los 2 modales "Ampliar" de `Company.jsx` (FinancialChart FCF/ingresos y RatioHistoryChart POC/POV vs precio) tenían el gráfico con altura fija (480/500px) y el modal centrado sin límite de alto → en horizontal se recortaban el título (arriba) y la leyenda (abajo, la del RatioHistory es `<Legend>` de recharts). FIX (mismo patrón que TrendModal): modal `flex flex-col max-h-[95vh] overflow-y-auto`, cabecera `shrink-0`, gráfico en contenedor de altura relativa (`h-[55vh] min-h-[240px] max-h-[520px]`, `h-[70vh]` en landscape de poca altura) con `renderChart("100%", true)` (renderChart ya acepta height="100%"). Verificado: título y leyenda visibles y modal completo dentro del viewport apaisado.
- **✅ Cabecera compacta en horizontal (3 ago 2026, verificado 900x400 y 1280x800):** en horizontal de poca altura (foldables/móviles apaisados) la cabecera mostraba el layout de escritorio (fila logo + fila de páginas + fila de utilidades) porque el ancho superaba el breakpoint `md` (768px), comiéndose el alto. FIX (`components/Layout.jsx`): estado JS `compact` = `innerWidth < 768 || (w>h && h<=600)` (listeners resize+orientationchange); cuando es `compact` se colapsa a UNA fila (logo + auth + botón ☰) y el resto (páginas + funciones: Ayuda/Umbrales/Idioma/Tema/Moneda) va al drawer del hamburguesa — igual que en vertical. Escritorio (no compact) mantiene las 2 filas. Verificado: horizontal 900x400 → cabecera 69px con ☰ que despliega todo; escritorio 1280x800 sin cambios.
...
- **✅ Visual: 3 ajustes UI (3 ago 2026, verificado screenshots+medidas):**
  ...
  3. **Tabla con 2 primeras columnas fijas** (`Visual.jsx`): casilla (`w-[29px]`, `sticky left-0`) y Ticker (`sticky left-[29px]`, `border-r`) congeladas con `z-30`(header)/`z-20`(body) y fondo opaco; el resto hace scroll horizontal. FIX hueco (3 ago): la casilla renderiza 29px pero el Ticker estaba en `left-[40px]` → hueco de 11px por el que se veían pasar las columnas; ajustado a `left-[29px]` → hueco 0px (verificado por medida).
