...
- **🔨 EN CURSO (Feb 2026) — Thesis Engine v2 (decisiones cerradas con el usuario, construir con E2+Opus):**
  ...
  - **✅ F2 — Probabilidad + contratesis (hecho):** círculo de probabilidad 0-10 (0 rojo → 10 verde) calculado por Claude con justificación al **hover** (`ProbabilityCircle.jsx` + `HoverTip`), arriba junto al título. Synthesizers devuelven `thesis_probability`/`overall_probability` + rationale. **Contratesis bajo demanda**: botón "Añadir contratesis" → `POST /api/thesis/{id}/contra` (requiere login; idempotente). Genera antítesis con búsqueda web fresca: resumen + su propio círculo de probabilidad + cadena de valor afectada + empresas más perjudicadas (con ticker) para tendencias, o `losing_trends` para empresas. Render: bloque salmón/rojo de baja presencia entre título y cadena de valor (`ContraSection` en `ThesisResult.jsx`). Backend: `run_trend_contra`/`run_company_contra`/`_contra_probability` en `services/thesis.py`. Verificado end-to-end (Robótica humanoide: tesis 6/10 + contra 6/10 con NVDA/ABB/FANUY/TER/CGNX) + screenshot. Nota: el render de contra para flujo EMPRESA (`losing_trends`) es simétrico pero solo verificado por código, no visualmente.
