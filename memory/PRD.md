...
- **✅ Visual: ejes invertidos en móvil (3 ago 2026, verificado screenshots 390px y 1280px):** en móvil (ancho <768px) el gráfico de cuadrante invierte los ejes a petición del usuario — **Score cualitativo en el eje vertical (Y)** y **Ratio Compra % en el eje horizontal (X)**; en escritorio se mantiene el original (Score en X, Ratio Compra en Y). Estado `isMobile` (dentro del efecto chartH). Adaptados: ejes XAxis/YAxis (dataKey/domain/label), `ReferenceLine` (medianas), `QuadrantLabels` (nueva prop `inverted` reposiciona PREMIUM↖/JOYAS↗/SOBREVALORADA↙/TRAMPA↘) y `tlTrails` (estelas de la línea de tiempo intercambian x/y). `Visual.jsx`.
