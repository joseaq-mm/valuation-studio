...
def effective_stage_tam(comp, value_chain, global_tam=None):
    ...
    Safety cap (C): a company's addressable market in a thesis can never exceed the
    thesis's whole market (global TAM, or the sum of all stage TAMs as fallback), so
    absorption/merges can't inflate the TAM Score artificially via double counting.
    ...
    if base is None and not absorbed:
        return None
    eff = (base or 0) + absorbed
    cap = global_tam or sum((s.get("tam_busd") or 0) for s in (value_chain or []))
    if cap and eff > cap:
        eff = cap
    return round(eff, 1)
...
GROWTH_DRIVER_MAPPER_SYS = (
    ...
    "INTERNACIONALIZACIÓN / EXPANSIÓN GEOGRÁFICA: la expansión a otros países de un negocio que ya constituye "
    "una tesis NO es un driver independiente (es la MISMA propuesta de valor en otra localización); NO la mapees "
    "como driver aparte. El TAM de cada tesis debe reflejar su mercado GLOBAL (incluido el de otros países), y la "
    "internacionalización se trata como evidencia/fase dentro de la tesis correspondiente, no como mercado nuevo. "
...
DRIVER_RECONCILER_SYS = (
    ...
    "(5) INTERNACIONALIZACIÓN / EXPANSIÓN GEOGRÁFICA: si un driver es la expansión a otros países/regiones de un "
    "negocio que YA está representado por otra(s) tesis (la MISMA propuesta de valor en otra localización, no un "
    "negocio distinto), NO lo dejes como tesis separada: REPARTE su TAM de forma PROPORCIONAL entre las tesis que "
    "se internacionalizan (conservando el total, sin doble conteo; típicamente porque el TAM de esas tesis debe "
    "reflejar el mercado GLOBAL, no solo el doméstico) y MENCIONA en el 'fit_description' de esas tesis que hay "
    "una internacionalización en marcha (mercados objetivo y fase). Trátala como tesis propia SOLO si la expansión "
    "abre un negocio realmente distinto (otra propuesta de valor/regulación/modelo), algo excepcional. "
...
async def _reconcile_drivers(company: str, drivers: list, sources_block: str, existing_theses: list = None) -> list:
    ...
    user = (
        ...
        "ordénalos por convicción. INTERNACIONALIZACIÓN: si un driver es solo la expansión geográfica de un "
        "negocio ya cubierto por otra tesis, NO lo dejes aparte: reparte su TAM proporcionalmente entre esas "
