...
// `items` = [{ key, label, value }] already expressed in `currency`.
// `blur` blurs the monetary total (privacy toggle from the header).
export const PortfolioDonut = ({ items, currency = "USD", title = "Composición de la cartera", testid = "portfolio-donut", blur = false }) => {
    const data = useMemo(() => {
        ...
        <div className="border border-black bg-white p-4" data-testid={testid}>
            ...
            <div className="flex flex-col items-center gap-4">
                ...
                <div className="w-full grid grid-cols-1 gap-y-1.5" data-testid={`${testid}-legend`}>
                    {data.map((d) => (
                        <div key={d.key} className="flex items-center gap-2 text-xs font-mono" data-testid={`${testid}-item-${d.key}`}>
